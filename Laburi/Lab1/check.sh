...
compile_java() {
  ...
  command pushd "$1" &>/dev/null
    if [[ ! ( -f bin/Main.class && bin/Main.class -nt src/Main.java ) ]]; then
      ...
      javac src/Main.java -d bin >> "$LOG_NAME" 2>&1
      result=$?
      echo "========================================" >> "$LOG_NAME" 2>&1
    fi
...
compile_cpp() {
  ...
  command pushd "$1" &>/dev/null
    if [[ ! ( -f main && main -nt main.cpp ) ]]; then
      echo "Compiling $1..." >> "$LOG_NAME" 2>&1
      ${CXX} ${CXXFLAGS} -o main main.cpp >> "$LOG_NAME" 2>&1
      result=$?
      echo "========================================" >> "$LOG_NAME" 2>&1
    fi
  command popd &>/dev/null
...
compile_and_run() {
  ...
  if [[ $result -eq 0 ]]; then
    command pushd "tests/${task}/checker" &>/dev/null
      make
    command popd &>/dev/null

    "tests/${task}/checker/main" "tests/${task}/${num}.ref" "${lang}/${task}/out" >> "$LOG_NAME" 2>&1
    if [[ $? -ne 0 ]]; then
      result=3
    fi
  fi
...
run_single_test() {
  ...
  compile_and_run "$lang" "$task" "$num" &>/dev/null
  result=$?
...
cleanup() {
  ...
  for task in ${TASKS[@]}; do
    ...
    cleanup_make "tests/${task}/checker"
  done
}
